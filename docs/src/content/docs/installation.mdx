...
<Tabs>
  ...
  <TabItem label="Deno (JSR)">
    ...
    **Permissions**: For more security, you can specify exact permissions instead of `-A`:
    ...
    :::note
    `--allow-ffi` enables optimized Copy-on-Write file cloning on macOS (APFS) and Linux (Btrfs/XFS). `--allow-net` is required for the `vibe upgrade` command to check for updates.
    :::
