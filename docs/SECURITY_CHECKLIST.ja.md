...
- **対策**: `validatePath()` によるパス境界の検証。git worktree のパスについては `validateWorktreePath()` が追加で、相対パス・`..` セグメント・制御文字・先頭の `-`（引数インジェクション対策）・Windows のドライブ相対 / ロングパス / UNC プレフィックスを拒否する。検証は生成元（`resolveWorktreePath`、`path_script` の出力、フックの stdin）と sink（`git worktree add/remove` 直前）の両層に適用する。
- **スコープ**: 本対策は構文レベルのバリデーションのみ。シンボリックリンク解決および許可ルートのホワイトリスト化は今後の拡張課題として管理する。
