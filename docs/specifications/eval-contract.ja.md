...
- ラッパー文字列と補完スクリプトの出力は、リリースをまたいでバイト単位で同一を保つこと（**MUST**）。ユーザーの rc ファイルに既に読み込まれているラッパーはアップグレード時に再生成されず、正確なバイト列に依存しています。
...
以下は現在の実装で観測されている制限です。ここでは修正方針を規定しません。

- **nushell**: ラッパーは stdout を行に分割し、各行を `nu -c` で実行します。出力される `cd` 行は POSIX のシングルクォート・エスケープを使いますが、これは nushell の文字列構文ではありません。したがってシングルクォートを含むパスは nushell では正しく扱われません。
- **powershell**: `Invoke-Expression` は PowerShell のクォート規則で行を解釈し、POSIX とは同様のずれがあります。同じクォート方言の注意点が当てはまります。
- シングルクォートを含まない通常のパスは、5 つのシェルすべてで正しく動作します。
...
ルール: stdout / stderr の分離に影響する変更は、`rust/crates/vibe/tests/eval_contract.rs` にケースを追加しなければなりません（**MUST**）。
...
- `rust/crates/vibe-core/src/stdin.rs`, `rust/crates/vibe/src/cli.rs` — Claude Code フックのプロトコル
