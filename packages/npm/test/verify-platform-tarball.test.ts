...
import { findTarballProblems } from "../../../scripts/verify-platform-tarball";
...
describe("findTarballProblems", () => {
  ...
  it("flags a non-executable binary", () => {
    const problems = findTarballProblems([
  ...
  it("reports every problem at once (empty tarball)", () => {
    const problems = findTarballProblems([{ path: "package.json", size: 50, mode: plainMode }]);
