/**
 * Bun filesystem implementation tests
 *
 * Tests that Node.js implementation works correctly in Bun runtime.
 */
...
import { nodeFS } from "../../../core/src/runtime/node/fs.ts";

let TEST_DIR: string;

beforeAll(async () => {
  TEST_DIR = await fs.mkdtemp(path.join(os.tmpdir(), "vibe_bun_fs_test_"));
});
...
describe("nodeFS in Bun", () => {
  ...
  describe("makeTempDir", () => {
    it("creates temporary directory", async () => {
      const tempDir = await nodeFS.makeTempDir({ prefix: "buntest_" });

      try {
        const stat = await fs.stat(tempDir);
        expect(stat.isDirectory()).toBe(true);
        expect(tempDir).toContain("buntest_");
      } finally {
