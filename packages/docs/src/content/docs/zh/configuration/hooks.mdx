...
`vibe start --claude-code-worktree-hook` 没有需要留在原地的 shell，因此 `pre_start`
...
该行在每次调用中最多出现一次，不带任何样式（没有颜色码），且仅在 `pre_start` 被闸门
拦下时出现；`post_start` 失败只会产生常规的 `Warning: Hook "..." failed: ...`。请把
它当作一个信号：你刚拿到的路径指向的 worktree 跳过了复制和 `post_start`。

有两点需要注意：

- 下文的[设置示例](#设置)在命令末尾使用了 `2>/dev/null`，这会丢弃 stderr，也就一并
  丢弃了该信号。如果你想观察到闸门，请去掉这个重定向。
- 该信号只伴随创建 worktree 的那次运行。如果对已经存在 worktree 的分支再次调用钩子，
  它会直接返回已有路径而不重新运行钩子，因此第二次调用不会输出该信号。
