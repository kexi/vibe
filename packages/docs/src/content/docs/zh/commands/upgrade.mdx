...
如果你的 `$SHELL` 是 nushell 或 PowerShell（或者你正在 Windows 上），并且调用 vibe 的包装函数不是当前版本，`upgrade` 会追加一行提示，引导你使用 [`vibe doctor`](/zh/commands/doctor/)。当包装函数已表明自己是当前版本时，这行提示会被跳过。
