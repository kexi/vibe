...
- 已被另一个条目包含的 `[copy] dirs` 条目不再重复复制。克隆 `a` 时 `a/b` 本来就会作为其一部分被放好，因此针对 `a/b` 的第二个条目会遇到已存在的目标：`clonefile` 以 `EEXIST` 失败，而 vibe 把它当作可忽略的失败处理，于是复制回退为普通的递归遍历，把克隆刚刚放好的内容又逐个文件重写了一遍。结果是正确的，也不会有任何警告，所以这份开销一直不可见。很普通的配置就会触发：`dirs = ["**/node_modules"]` 会展开为根目录的 `node_modules` 加上包管理器留在其中的每一个嵌套副本，在一个有代表性的仓库中共 31 个条目，其中 27 个都包含在根目录之内。在 360MB、17,789 个文件上实测，`vibe start` 从 3.43 秒降至 0.48 秒，生成的两个 worktree 完全一致。未被其他条目包含的条目不受影响。
...
- 在 macOS 上，嵌套的 `[copy] dirs` 条目现在也能用上写时复制（CoW）克隆了。当目标的父目录不存在时，`clonefile(2)` 会以 `ENOENT` 失败，而 vibe 把它当作可忽略的失败处理，于是复制回退为完整的递归拷贝，却仍然报告成功、进度阶段也仍然标着 `clonefile`。受影响的是那些中间目录在新建 worktree 中尚不存在的条目，也就是 `packages/app/node_modules` 这类 monorepo 结构。在 APFS 上以 360MB、17,789 个文件的 `node_modules` 实测，`vibe start` 从 2.71 秒降至 0.41 秒。父目录已存在的条目原本就不受影响，行为保持不变。
...
- `vibe list` 新增 BASE、AGE 和 STATUS 列，无需逐个 `cd` 进去，就能把被搁置的 dirty worktree 和正在进行的工作区分开。同样的值也追加到 `--json` 中，输出为 `name`、`base`、`head`、`last_commit_at`、`status` 和 `dirty_files`；无法确定的值会降级为 `null`，而不会让整个列表失败。
- `vibe list` 新增筛选与排序：`--dirty` / `--clean`、`--base`、`--recent` / `--stale`、`--limit`、`--sort age|name|status` 和 `--reverse`。筛选条件以 AND 组合并在排序之前应用，`--reverse` 在 `--limit` 之前生效，因此 `--sort age --reverse --limit 5` 表示「最旧的 5 个」。默认的最近使用顺序保持不变。
- `.vibe.toml` 中新增可选的 `[summary]` 段，让 `vibe list` 显示说明每个 worktree 用途的 SUMMARY 列。vibe 会把未命中缓存的 worktree 以 JSON 形式通过标准输入传给你配置的命令，并从标准输出读取 `{name: summary}`。该命令是任意 shell 命令，因此同样受既有的 SHA-256 信任门控约束：修改它会撤销信任。基于内容寻址的缓存以每个 worktree 的 HEAD 和 status 为键，因此在没有变化时列表不会启动任何进程。
- 当 `pre_start` 拦截了 worktree 时，`vibe clean --claude-code-worktree-hook` 现在会向标准错误写入一行固定内容，让代理获得人类从被抑制的 `cd` 中得到的同样信息。标准输出的内容和退出码 0 保持不变。

### 变更

- **破坏性变更：** 在此前只被解析、实际被忽略的位置上使用 `*_prepend` / `*_append` 的 `.vibe.toml`，现在需要重新执行 `vibe trust`。让这些位置生效，意味着把休眠的配置变成会被执行的配置；而信任哈希覆盖的是文件的字节，而非 vibe 如何解释它们 —— 因此仅仅升级二进制，就可能让一条从未运行过的 `post_start_append` 开始运行。vibe 现在会在每条信任记录旁记录配置语义修订号，并在检测到受影响的字段时明确列出字段名并安全地失败。未使用这些位置的配置不受影响。

### 移除

- **破坏性变更：** 移除了 v3.1.0 添加的默认分支保护，以及 `vibe clean` 和 `vibe rename` 上的 `--allow-default-branch`；再传入该选项会报错。该保护通过 `refs/remotes/origin/HEAD` 推断默认分支，而这个 ref 由 `git clone` 写入，之后 `git fetch` 并不会更新它 —— 要刷新它需要显式运行 `git remote set-head origin --auto`。在本地 109 个仓库上的实测中，它有 7 次与远程的真实默认分支不一致，且每一次都是同一种情况：仓库的默认分支已经移到 `develop`，而 `origin/HEAD` 仍然写着 `main`。也就是说，它恰恰在最需要它的工作流上失效，却仍然宣称提供了保护。未合并的分支、以及在其他 worktree 中检出的分支，`git branch -d` 依然会拒绝删除；已推送的分支，`vibe rename` 依然会拒绝重命名 —— 这覆盖了所有共享的默认分支。
