...
out of a worktree it could not provision. A gated `vibe start` still leaves the
worktree directory on disk; only entering it is withheld, so you can fix the
cause and re-run `vibe start`.
