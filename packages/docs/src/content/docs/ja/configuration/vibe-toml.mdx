...
`path_script` に渡される環境変数（`VIBE_REPO_NAME`、`VIBE_BRANCH_NAME`、
`VIBE_SANITIZED_BRANCH`、`VIBE_REPO_ROOT`）は、ブランチ名など
ユーザー制御の値に由来します。シェルインジェクションを防ぐため、
スクリプト内では以下のルールを必ず守ってください：

- **環境変数は必ずダブルクォートで囲む**。クォートされていない `${VAR}`
  はバグとして扱ってください：
  ...
  # 悪い例：攻撃者が制御できる値で word-splitting / glob 展開が起こる
  echo $HOME/worktrees/$VIBE_REPO_NAME-$VIBE_SANITIZED_BRANCH
  ```

- これらの変数に対して **`eval` を絶対に使わない**。値を再解釈してしまう
  `bash -c "... $VAR ..."` のようなパターンも避けてください。

vibe はスクリプトの実行前後で多層的な防御を行います：

| レイヤー                               | 動作                                                                                     |
| -------------------------------------- | ---------------------------------------------------------------------------------------- |
| 環境変数値に含まれる制御文字           | **拒否**（スクリプトは実行されません）。`git branch -m` でブランチ名を変更してください。 |
| 環境変数値に含まれるシェルメタ文字     | stderr へ**警告**を出力した上で実行は継続。スクリプトのクォートを修正してください。      |
| スクリプトの標準出力に含まれる制御文字 | **拒否**。スクリプトは 1 行の絶対パスのみを出力する必要があります。                      |
