...
Claude Code WorktreeRemoveフックモード。stdinからWorktreeパスを読み取り、Worktreeを削除します。

```bash
vibe clean --claude-code-worktree-hook --force
```

設定の詳細は[Claude Code連携](/ja/configuration/hooks/#claude-code-integration)を参照してください。
...
- **Linux**: XDGゴミ箱に移動します。利用できる環境では、デスクトップのファイルマネージャーから復元できます。
- **Windows**: ごみ箱に移動します。必要に応じて復元可能で、OSの動作と一貫したクリーンアップになります。

システムのゴミ箱が利用できない、または失敗した場合、vibe はシステムの一時ディレクトリ（`/tmp`または`%TEMP%`）に worktree を移動し、バックグラウンドで削除する処理にフォールバックします。一時ディレクトリはシステム再起動時に自動的にクリーンアップされます。

**詳細なプロセス:**

1. worktreeディレクトリは瞬時に移動（O(1)リネーム操作）
