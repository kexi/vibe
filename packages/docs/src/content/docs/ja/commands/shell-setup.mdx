...
ラッパー関数の後ろにシェル補完スクリプトを追加します。現状は `fish` と `zsh` をサポート。`bash` / `nushell` / `powershell` で指定するとエラー終了します。
...
> zsh の場合は、`eval` 行より前に `~/.zshrc` で `autoload -Uz compinit && compinit` を呼んでおく必要があります。`compinit` が実行されていない場合は補完登録が silently スキップされ、`vibe` ラッパー関数のみが定義されます。
...
### Nushell / PowerShell のラッパー

vibe 2.2.0 以降、`nushell` と `powershell` 向けに出力されるラッパーは内部フラグ `--eval-dialect` を
渡します。これによりバイナリはディレクトリ変更をそのシェル自身の構文で出力します。したがってこの
2 つのラッパーは対応するバイナリ（2.2.0 以降）を必要とします。古いバイナリに対してはフラグが拒否され、
何もせずに終了コード 2 で終わります。

rc ファイルに既に書き込まれたラッパーは、アップグレード時に再生成され**ません**。古い nushell /
powershell のスニペットを貼り付けている場合は、`vibe shell-setup` を実行し直して置き換えるまで
旧来の挙動のままです（nushell ではディレクトリが変わらず、PowerShell ではクォートを含むパスを
誤って扱います）。`bash` / `zsh` / `fish` のラッパーは変更されていません。
