...
※`--base`は新規ブランチ作成時のみ有効です。既存ブランチの場合は無視されます。デフォルトでは`--base`はupstreamを設定しません。upstreamを設定するには`--track`を使用してください。

※`--claude-code-worktree-hook`はClaude CodeのWorktreeCreateフックから内部的に使用されます。設定の詳細は[Claude Code連携](/ja/configuration/hooks/#claude-code-integration)を参照してください。
