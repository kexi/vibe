...
3. **Partial match (single)**: If exactly one worktree contains the input as a substring, navigate to it
4. **Partial match (multiple)**: If multiple worktrees match, display an interactive selection prompt. If no case-sensitive partial matches are found, a case-insensitive search is performed as a fallback
5. **No match**: Offer to create a new worktree using `vibe start`
