...
Any cell whose value could not be determined renders as `-`. A worktree that git cannot read (for example one left behind by a deleted checkout) still appears as a row; only the affected cells degrade, and the reason is reported as a warning.
...
`--base` compares against the resolved [BASE](#base) column, after stripping a remote prefix from **both** sides — so `--base develop` and `--base origin/develop` select the same rows. The match is exact, not a prefix: `--base develop` does not match `develop-2`. A worktree with no base (the main worktree, or a detached HEAD) is never matched by `--base`.
...
| `head`           | string \| null | The commit sha HEAD points at; `null` when git reported none                |
