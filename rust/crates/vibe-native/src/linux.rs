...
/// Directory cloning is NOT supported by FICLONE (files only). Returns an
/// [`CloneError::Unsupported`] so the caller falls back to another strategy —
/// distinct from `UnsupportedFileType`, which is a hard error.
pub fn clone_directory(_src: &Path, _dest: &Path) -> CloneResult<()> {
    Err(CloneError::Unsupported(
        "FICLONE does not support directories on Linux",
    ))
}
...
mod tests {
    ...
    // G-18: on Linux, the public `clone_directory` is NOT supported (FICLONE is
    // files-only). It must return the SOFT `Unsupported` error (so the copy
    // executor falls back to another strategy) — distinct from the HARD
    // `UnsupportedFileType` returned by `clone_file` on a directory.
    #[test]
    fn test_clone_directory_unsupported_on_linux() {
        let temp_dir = TempDir::new().unwrap();
        let src_dir = temp_dir.path().join("src_dir");
        let dest_dir = temp_dir.path().join("dest_dir");
        fs::create_dir(&src_dir).unwrap();
        ...
        assert!(
            matches!(result, Err(CloneError::Unsupported(_))),
            "Linux clone_directory must return a soft Unsupported error: {result:?}"
        );
    }
}
