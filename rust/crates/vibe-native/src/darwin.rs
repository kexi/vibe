...
/// Clone a directory using macOS clonefile() — the same syscall as for files
/// (clonefile supports directories on APFS).
...
mod tests {
    ...
    // G-18: on macOS the public `clone_directory` clones a directory via
    // clonefile (the same syscall as files). On a non-APFS test filesystem it may
    // return ENOTSUP — both are acceptable; the point is it does NOT reject the
    // directory as an unsupported file type (unlike Linux's files-only FICLONE).
    #[test]
    fn test_clone_directory_on_macos() {
        let temp_dir = TempDir::new().unwrap();
        let src_dir = temp_dir.path().join("src_dir");
        let dest_dir = temp_dir.path().join("dest_dir");
        fs::create_dir(&src_dir).unwrap();
        fs::write(src_dir.join("inner.txt"), "hello").unwrap();

        match clone_directory(&src_dir, &dest_dir) {
            Ok(()) => {
                assert!(dest_dir.is_dir());
                assert_eq!(
                    fs::read_to_string(dest_dir.join("inner.txt")).unwrap(),
                    "hello"
                );
            }
            // Non-APFS test volume: clonefile is unsupported there.
            Err(CloneError::SystemError { errno, .. }) if errno == libc::ENOTSUP => {}
            Err(e) => panic!("unexpected clone_directory error: {e:?}"),
        }
    }
